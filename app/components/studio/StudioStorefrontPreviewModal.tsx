...
export default function StudioStorefrontPreviewModal({
  ...
  const activeFields = activeScreen?.fields || fields || [];
  ...
  useEffect(() => {
    ...
    activeFields.forEach((f) => {
      if (initialVals[f.id] === undefined) {
        ...
        } else if (f.fieldType === "SELECT" || f.fieldType === "RADIO" || f.fieldType === "COLOR_SWATCH" || f.fieldType === "IMAGE_SWATCH") {
          const opts = f.config?.options || [];
          initialVals[f.id] = opts[0]?.value || opts[0]?.label || "";
        }
      }
    });

    setFormValues(initialVals);
  }, [isOpen, activeScreenIndex]);
  ...
  // Initialize Fabric Canvas for Live Preview
  useEffect(() => {
  ...
  // Render Fabric Objects for Live Preview whenever Form Values or Active Screen Changes
  useEffect(() => {
    ...
    sortedLayers.forEach((layer) => {
      ...
      // Determine value to display (customized by customer OR default)
      ...
      if (layer.layerType === "TEXT") {
        ...
        if (layer.linkedFieldId && formValues[layer.linkedFieldId]) {
          assetUrl = formValues[layer.linkedFieldId];
        }
  ...
  // Number of Grandkids / Screen Selector Buttons
  const screenFieldLabel = screenFieldConfig?.customerLabel || "Number Of Grandkids";
  const showScreenNumberGrid = screenFieldConfig?.enableScreenField || screens.length > 1;
        ...
        <div className="px-6 py-3.5 bg-slate-900 text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            <span className="bg-blue-600 text-white font-bold text-xs px-2.5 py-1 rounded-full flex items-center gap-1">
              ...
              <span>Shopify Storefront Live Preview</span>
            </span>
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-slate-200">
                ...
                <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-thin">
                  {screens.map((scr, idx) => (
                    <button
                      ...
                      className={`relative shrink-0 w-16 h-16 rounded-lg border-2 overflow-hidden transition cursor-pointer bg-white shadow-xs ${
                        activeScreenIndex === idx ? "border-blue-600 ring-2 ring-blue-600/30 scale-105" : "border-slate-200 hover:border-slate-300 opacity-75 hover:opacity-100"
                      }`}
                    >
                      {scr.bgUrl ? (
                        <img src={scr.bgUrl} alt={scr.name} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex flex-col items-center justify-center bg-slate-100 text-[10px] font-bold text-slate-600 p-1 text-center">
                          <span>Screen</span>
          ...
          <div className="lg:col-span-5 p-6 bg-white flex flex-col justify-between space-y-6">
            <div className="space-y-6">
              ...
              {/* 1. NUMBER OF GRANDKIDS / SCREEN SELECTOR GRID */}
              {showScreenNumberGrid && (
                <div className="space-y-2">
                  ...
                  {/* GRID OF NUMBER BUTTONS (1 TO 20) */}
                  <div className="grid grid-cols-6 gap-2">
                    {Array.from({ length: Math.max(screens.length, 12) }).map((_, idx) => {
                      const screenForIndex = screens[idx] || screens[0];
                      ...
                      return (
                        <button
                          ...
                          className={`relative h-12 rounded-xl border-2 font-black text-lg transition flex items-center justify-center cursor-pointer shadow-2xs ${
                            isSelected
                              ? "bg-white border-blue-600 text-slate-900 ring-2 ring-blue-600/30 scale-105"
                              : idx < screens.length
                        ...
                        >
                          <span>{idx + 1}</span>
                          {isSelected && (
                            <span className="absolute bottom-1 right-1 w-3.5 h-3.5 bg-blue-600 rounded-full flex items-center justify-center text-white">
                          ...
                          {(field.config?.options || []).map((opt: any, idx: number) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg border text-xs font-bold transition flex items-center gap-1.5 cursor-pointer ${
                                  isSel
